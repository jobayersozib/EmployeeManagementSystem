...
public class Database implements ActionListener{
   ...
   public Database() {
    ...
    public void submitAttendance(JTextField employeeId,JTextField date,JComboBox status){
        this.sql="INSERT INTO `employee_attendance`(`id`, `attendance_status`, "
                + "`date`, `attendance_given`) "
                + "VALUES (?,?,?,?)";
        
        try {
            this.pstm=this.cn.prepareStatement(this.sql);
            this.pstm.setString(1,employeeId.getText());
            this.pstm.setString(2,date.getText());
            this.pstm.setString(3,(String)status.getSelectedItem());
            this.pstm.setString(4,UserInfo.userId);
            int check=this.pstm.executeUpdate();
            if(check==1){
            
                JOptionPane.showMessageDialog(null,"Submit success","Success",JOptionPane.PLAIN_MESSAGE);
